...
import "./types.js";
import { r as CAPNWEB_TRANSPORT_VALUE, t as CAPNWEB_TRANSPORT_QUERY } from "./transport-protocol-BxyngQ_R.js";
import { c as nativeCall, i as NativeCallQueue, l as splitCallOptions, n as AgentConnectionError, o as createStubProxy, s as isTerminalCloseEvent, u as boundCapnWebSocket } from "./client-C9DAjZIY.js";
...
function useAgent(options) {
	...
	const { query, queryDeps, cacheTtl, sub: subOption, path: userPath, defaultCallTimeout, onConnectionError, shouldReconnectOnClose, transport = "cf-websocket", ...restOptions } = options;
	const capnWebRef = useRef(null);
	const transportRef = useRef(transport);
	transportRef.current = transport;
	const nativeQueueRef = useRef(new NativeCallQueue());
	const socketClass = useMemo(() => transport === "capnweb" ? { WebSocket: boundCapnWebSocket((socket) => {
	...
	const flushQueuedCalls = () => {
		...
		const native = transportRef.current === "capnweb" ? capnWebRef.current : null;
	...
	const socketQuery = transport === "capnweb" ? {
		...resolvedQuery,
		[CAPNWEB_TRANSPORT_QUERY]: CAPNWEB_TRANSPORT_VALUE
	} : resolvedQuery;
	...
	const call = useCallback((method, args = [], options) => {
		if (transportRef.current === "capnweb") {
			...
			const socket = socketRef.current;
			if (native && socket?.readyState === WebSocket.OPEN) return nativeCall(native, method, args, options, defaultCallTimeoutRef.current);
			if (socket && connectionErrorRef.current && socket.readyState === socket.CLOSED) return Promise.reject(/* @__PURE__ */ new Error("Connection closed"));
			return nativeQueueRef.current.enqueue(method, args, options, defaultCallTimeoutRef.current);
		}
